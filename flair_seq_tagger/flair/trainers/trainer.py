...
from flair.training_utils import (
    init_output_file,
    # Canasai's comment out: WeightExtractor,
    log_line,
    # Canasai's comment out: add_file_handler,
    Result,
    store_embeddings,
)
import random
...
class JointModelTrainer:
    def __init__(
        self,
        model1: flair.nn.Model,
        model2: flair.nn.Model,
        corpus: Corpus,
        optimizer: torch.optim.Optimizer = SGD,
        epoch: int = 0,
        use_tensorboard: bool = False,
    ):
        self.model1: flair.nn.Model = model1
        self.model2: flair.nn.Model = model2
...
class ModelTrainer:
    ...
    def train(
        ...
        **kwargs,
        ...
        # Canasai's comment out: weight_extractor = WeightExtractor(base_path)
        ...
        try:
            ...
            for self.epoch in range(self.epoch + 1, max_epochs + 1):
                ...
                log_line(log)
                log.info(
                    f"EPOCH {self.epoch} done: loss {train_loss:.5f} - lr {learning_rate:.5f}"
                )
                ...
                if (
                    ...
                    log.info(f"BEST SO FAR: {scheduler.best}")
        ...
        return {
            # Canasai's comment out: "test_score": final_score,
            "dev_score_history": dev_score_history,
            "train_loss_history": train_loss_history,
            "dev_loss_history": dev_loss_history,
        }
