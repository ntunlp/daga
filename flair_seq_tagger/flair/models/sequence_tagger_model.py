...
# Canasai's addition:
from flair.attentions import MultiHeadedAttention, SoftAttention
...
class SequenceTagger(flair.nn.Model):
    ...
    def _get_state_dict(self):
        model_state = {
            ...
            "use_sent_query": self.use_sent_query,
    ...
    def _init_model_with_state_dict(state):
        ...
        model = SequenceTagger(
            ...
            column_format=state["column_format"],
            use_attn=state["use_attn"],
            attn_type=state["attn_type"],
            scaling=state["scaling"],
            pooling_operation=state["pooling_operation"],
            use_sent_query=state["use_sent_query"],
    ...
    def forward_loss(
        self, data_points: Union[List[Sentence], Sentence], memory_bank=None, sort=True
    ) -> torch.tensor:
        features = self.forward(data_points, memory_bank)
        return self._calculate_loss(features, data_points)

    def forward(self, sentences: List[Sentence], memory_bank=None):
        if memory_bank is not None:
            features = self.linear(memory_bank)
            return features
        ...
        for sentence in sentences:
            ...
            if nb_padding_tokens > 0:
                ...
                all_embs.append(t)
                sent_mask.append(([1] * len(sentence)) + [0] * nb_padding_tokens)
            else:
                sent_mask.append([1] * longest_token_sequence_in_batch)
        ...
        # sent_mask: batch x len
        sent_mask = torch.tensor(sent_mask, dtype=torch.float, device=flair.device,)
        ...
        if self.use_rnn:
            ...
            memory_bank, encoder_final = self.rnn(packed)

            memory_bank, output_lengths = torch.nn.utils.rnn.pad_packed_sequence(
                memory_bank, batch_first=True
            )

            if isinstance(encoder_final, tuple):  # LSTM
                encoder_final = encoder_final[0]
            ...
            if self.bidirectional:
                encoder_final = torch.cat(
                    [
                        encoder_final[0 : encoder_final.size(0) : 2],
                        encoder_final[1 : encoder_final.size(0) : 2],
                    ],
                    2,
                )
            encoder_final = encoder_final[-1]  # use the last layer

            if self.locked_dropout:
                memory_bank = self.locked_dropout(memory_bank)
        else:
            encoder_final = None
            memory_bank = sentence_tensor

        if self.self_attn:
            # sent_mask: batch x 1 x len
            sent_mask = sent_mask.unsqueeze(-2)
            attn_out = self.self_attn(memory_bank, memory_bank, memory_bank, sent_mask)
        elif self.soft_attn:
            # sent_mask: batch x len x 1
            sent_mask = sent_mask.unsqueeze(-1)
            attn_out = self.soft_attn(memory_bank, sent_mask)
        else:
            attn_out = None

        if attn_out is not None:
            if self.locked_dropout:
                attn_out = self.locked_dropout(attn_out)
            memory_bank = memory_bank + attn_out

        features = self.linear(memory_bank)
        self.memory_bank = memory_bank

        # encoder_final is further used by classifier
        if self.pooling_operation == "mean":
            self.encoder_final = memory_bank.mean(1)
        elif self.pooling_operation == "max":
            self.encoder_final, _ = torch.max(memory_bank, 1)
        else:
