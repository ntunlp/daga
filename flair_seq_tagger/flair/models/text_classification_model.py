...
class TextClassifier(flair.nn.Model):
    ...
    def forward(self, sentences, encoder_final=None) -> List[List[float]]:
        if encoder_final is not None:
            return self.decoder(encoder_final)

        self.document_embeddings.embed(sentences)

        label_scores = self.decoder(self.document_embeddings.encoder_final)
    ...
    def forward_loss(
        self, data_points: Union[List[Sentence], Sentence], encoder_final=None,
    ) -> torch.tensor:

        scores = self.forward(data_points, encoder_final)
