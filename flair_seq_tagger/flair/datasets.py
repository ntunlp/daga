...
class ColumnCorpus(Corpus):
    def __init__(
        ...
        # else:
            ...
            log.info(f"Sample dev data from train data")
...
class ClassColumnCorpus(Corpus):
    def __init__(
        self,
        data_folder: Union[str, Path],
        column_format: Dict[int, str],
        ...
        super(ClassColumnCorpus, self).__init__(train, dev, test, name=data_folder.name)
...
# Canasai's addition begin:
class FlyClassificationCorpus(ClassificationCorpus):
    def __init__(
        self,
        data_folder: Union[str, Path],
        train_file=None,
        ...
        new_train_file, new_dev_file, new_test_file = None, None, None
        if train_file is not None:
            train_file = data_folder / train_file
            new_train_file = train_file.with_suffix(".cls.txt")
        if test_file is not None:
            test_file = data_folder / test_file
            new_test_file = test_file.with_suffix(".cls.txt")
        if dev_file is not None:
            dev_file = data_folder / dev_file
            new_dev_file = dev_file.with_suffix(".cls.txt")

        orig_files = [train_file, dev_file, test_file]
        new_files = [new_train_file, new_dev_file, new_test_file]
        for orig_file, new_file in list(zip(orig_files, new_files)):
            if orig_file is None:
                continue
            if new_file.exists():
                log.info(f"{new_file} exists")
                continue
            self._convert_one_file(orig_file, new_file, comment_symbol, label_symbol)

        super(FlyClassificationCorpus, self).__init__(
            data_folder,
            train_file=new_train_file,
            test_file=new_test_file,
            dev_file=new_dev_file,
            tokenizer=tokenizer,
            max_tokens_per_doc=max_tokens_per_doc,
            max_chars_per_doc=max_chars_per_doc,
            in_memory=in_memory,
    ...
    def _convert_one_file(self, orig_file, new_file, comment_symbol, label_symbol):
        log.info("Converting data from {} to {}".format(orig_file, new_file))
        with io.open(orig_file, "r", encoding="utf-8", errors="ignore") as fin:
            with io.open(new_file, "w", encoding="utf-8", errors="ignore") as fout:
                ...
                for line in fin:
                    ...
                    if len(cols) == 2:
                        ...
                        assert buf and label
                        buf = [label] + buf
                        fout.write(" ".join(buf) + "\n")
                        fout.flush()
                        buf = []
                        label = None
                    elif comment_symbol is not None and cols[0].startswith(
                        comment_symbol
                    ):
                        continue
                    elif label_symbol is not None and cols[0].startswith(label_symbol):
                        label = cols[0]
                    else:
...
# Canasai's addition begin:
class ClassColumnDataset(ColumnDataset):
    def __init__(
        self,
        path_to_column_file: Path,
        column_name_map: Dict[int, str],
        ...
        super(ClassColumnDataset, self).__init__(
            path_to_column_file,
            column_name_map,
            tag_to_bioes,
            comment_symbol,
        ...
        with open(str(self.path_to_column_file), encoding=self.encoding) as f:
            ...
            while line:
                ...
                if self.label_symbol is not None and line.startswith(label_symbol):
                    label = line.strip().replace(label_symbol, "")
                    labels.append(label)
                    line = f.readline()
                    continue
            ...
            while line:
                ...
                if line == "":
                    ...
                    for morph in str(fields[5]).split("|"):
                        # Canasai's comment out: if not "=" in morph:
                        if "=" not in morph:
                            continue
...
def _download_wikiner(language_code: str, dataset_name: str):
    ...
    if not data_file.is_file():
        ...
        # Canasai's comment out: import bz2, shutil
        import bz2
